...
* 세마포어: 자원을 사용할 수 있는 횟수로 자원을 잠금. 프로세스가 자원에 접근할 때마다 횟수를 차감한다. 0이 되면 이후의 프로세스는 먼저 온 프로세스가 잠금을 풀어서 횟수가 늘어날 때까지 대기한다.
* 뮤텍스: 바이너리 세마포어와 비슷함. 대신 뮤텍스를 잠근 프로세스만이 잠금해제 할 수 있음. ex) 잠근 프로세스의 PID 값을 저장하는 방법을 씀.
* 모니터: 위의 세마포어나 뮤텍스 같은 락을 프로그래밍 언어에서 사용하기 쉽게 추상화된 형태로 제공해주는 것이다. 예로 들어 Java에선 `synchronized`라는 문법으로 제공된다.
...
* 지속성: 트랜잭션이 성공적으로 완료되었다면, 그 결과는 향후에 발생한 소프트웨어/하드웨어 장애에 의해서 바뀌어선 안됨. 즉, 커밋된 트랜잭션을 복구할 수단이 반드시 있어야 함.
...
격리 수준은 다른 사용자나 시스템에 보이는 트랜잭션의 무결성을 말한다.  
낮은 격리 수준에선 많은 사용자들이 동시에 같은 데이터를 볼 수 있지만 dirty reads나 lost updates 같은 동시성 문제를 겪을 수 있다.  
반대로 높은 격리 수준에선 이러한 동시성 문제는 줄어들지만 시스템 자원을 더 써야 하고, 다른 트랜잭션에 의해 block될 가능성이 있다.

이러한 수준을 두는 이유는 위에서 말했듯이 너무 격리 수준이 높으면 단점이 존재하기 때문에 시스템에 따라 균형을 맞추기 위함.  

격리 수준은 내림차순으로 다음과 같다.  

* Serializable: 모든 작업을 하나의 트랜잭션에서 처리하는 것 처럼 함. 이 방법을 이해하려면 공유 락, 배타 락 개념을 이해해야 함.
* Repeatable reads: 한 트랜잭션 안에서는 쿼리의 결과가 바뀌지 않음을 보장함. 하지만 다른 트랜잭션의 삽입/수정에 의해 현재 트랜잭션이 커밋된 이후에 같은 쿼리를 하면 다른 결과를 볼 수 있음. 이를 Phantom read라고 함.
* Read committed: 커밋이 완료된 데이터만 쿼리에서 보이는 걸 보장함. 하지만 쿼리와 쿼리 사이에 다른 트랜잭션이 커밋할 경우, 데이터가 바뀔 수 있음. 이를 Non-repeatable read라고 함.
* Read uncommitted: 쿼리 시 다른 트랜잭션이 커밋하지 않은 데이터를 읽을 수 있음. 이걸 dirty reads라고 부름

격리 수준에 따라 동일한 쿼리 결과를 보장하기 위해 DB의 현재 시점이 아닌 특정 시점의 DB 스냅샷을 읽어온다. 이를 Consistent read라 부른다.

#### 공유 락과 배타 락의 차이는 무엇인가요?  

공유 락: 읽기 잠금. 서로 다른 트랜잭션이 읽기 위해서 여러 개의 공유 락을 거는 건 되지만 변경하기 위한 배타 락은 안됨.  
배타 락: 쓰기 잠금. 어떤 트랜잭션에서 데이터를 변경하려고 하면 해당 트랜잭션이 완료될 때까지 테이블이나 레코드에 거는 락, 다른 트랜잭션은 배타 락이 걸린 곳은 공유 락을 걸 수 없음.

#### 데드락이란 무엇이며, 어떻게 발생할까요?  

(더 찾아봐야 함.)  
ex) 트랜잭션 A, B가 있을 때, A와 B가 공유 락을 먼저 걸고 배타락을 걸려고 하면 데드락 발생.
...
어떤 알고리즘에 대해서 입력값이 늘어날 수록 시간이 얼마나 오래 걸리는지, 공간이 얼마나 필요한 지를 수학식으로 표현한 것.

#### BFS/DFS 차이는 무엇인가요?  

BFS: 같은 depth에 접근 가능한 노드들을 전부 탐색한 후, depth를 증가시키는 방법으로 탐색.  
DFS: 시작 노드부터 더 이상 내려갈 노드가 없을 때까지 내려가면서 탐색하는 방법.  
...
<https://xlinux.nist.gov/dads/HTML/inplacesort.html>
<https://xlinux.nist.gov/dads/HTML/stable.html>

* In-place sort: 메모리 공간을 추가로 사용하지 않아도 되는 정렬법 ex) quicksort, insertion sort,...
* Stable: 입력된 배열에서 같은 키를 가지는 개체들의 순서가 정렬 후에도 변하지 않음
...
배열: 랜덤 엑세스가 O(1), 삽입과 삭제가 O(n), 노드 간에 에지가 필요 없음. 사이즈를 늘리려면 O(n)이 걸림.
링크드리스트: 랜덤 엑세스가 O(n), 삽입과 삭제가 O(1), 노드 간의 에지도 저장해야 함. 사이즈 늘리는데 O(1)이 걸림.

#### 스택과 큐에 대해서 설명해 주세요  

스택: 맨 끝에 삽입하고 맨 끝부터 삭제가 가능한 컬렉션
큐: 맨 끝에 삽입하고 맨 앞부터 삭제가 가능한 컬렉션

#### 해시테이블에 대해서 설명해 주세요  

key의 해쉬를 인덱스로 해서 value를 랜덤 엑세스 할 수 있는 컬렉션.  
실제로 저장할 key-value 집합의 수에 비해서 key의 범위가 너무 크다면 배열 대신 해쉬를 사용해 범위를 좁힌 해시테이블을 사용한다.  
범위가 좁아진 것 때문에 다른 key가 같은 hash를 가지는 충돌이 발생할 수 있는데, 이는 Open addressing나 chaining 방법을 쓴다.  

### 트리

#### 그래프와 트리의 차이점에 대해서 설명해 주세요  

그래프에서 모든 노드가 이어져 있지만 두 노드 간의 에지가 하나밖에 없고, 사이클이 없으면 트리
...
절차적 프로그래밍에선 순차적 실행에 초점을 맞춰서 코드를 작성하는 걸 의미함. 구현 대상을 실행 순서로 구분해서 각 파트를 코드로 구현하는 것.  
따라서 함수를 재사용하고 데이터는 프로시저 콜 사이의 중간자(매개변수나 반환값) 역할로 구분되어 있음.

객체지향은 객체 간의 관계에 초점을 두고 코드를 작성함. 구현 대상을 이루는 요소들의 역할과 상태, 행동을 먼저 정의하고 실행 순서를 객체 간의 관계 형태로 작성함.  

#### 객체지향 SOLID 원칙에 대해서 설명해 주세요

객체지향 프로그래밍 설계의 5가지 원칙

* 단일 책임 원칙: 한 클래스는 하나의 책임만 가져야 한다.
* 개방 폐쇄 원칙: 소프트웨어의 요소는 확장엔 열려 있으나 변경에는 닫혀 있어야 한다.
* 리스코프 치환 원칙: 상위 객체는 자기 속성을 바꾸지 않고 하위 객체로 변환할 수 있어야 함. (cf. 정사각형-직사각형 문제)
* 인터페이스 분리 원칙: 하나의 범용 인터페이스 보다 구체적이고 작게 분리된 여러 개가 낫다.
* 의존 관계 역전 원칙: 상위 모듈과 하위 모듈은 둘 다 추상화에 의존해야 한다.

#### 객체지향 4가지 특징에 대해서 설명해 주세요

<https://www.educative.io/blog/object-oriented-programming>

* 상속: 자식 클래스는 부모 클래스로부터 데이터와 행동을 상속받음.
* 캡슐화: 객체가 가지고 있는 정보 중 일부 정보만 공개함.
* 추상화: 객체에 접근할 수 있도록 고수준의 공개 메소드만 노출됨.
* 다형성: 여러 개의 메서드가 같은 작업을 수행할 수 있음. -> 메서드 오버라이딩, 오버로딩

#### 오버로딩과 오버라이딩의 차이는 무엇인가요?  

* 메소드 오버로딩: 같은 이름이지만 다른 매개변수를 필요로 하는 메서드를 선언함.  
* 메소드 오버라이딩: 부모 클래스의 메서드를 자식 클래스에서 재정의하는 것.
...
부수 효과가 없는 함수로만 코드를 작성하는 프로그래밍 패러다임.

#### AOP란 무엇인가요?  

프로그램의 여러 곳에 영향을 미치거나 의존하는 횡단 관심사를 분리해서 모듈화 하는 프로그래밍 패러다임.
...
실행 중인 Java 프로그램이 자기 자신을 조사하고 내부 속성을 수정할 수 있게 해주는 기능.
...
컨테이너 가상화 어플리케이션 중 하나.

가상화는 특정 어플리케이션의 실행 환경을 격리하기 위해 사용함. 사용할 자원량을 제한하고, 장애 시 시스템 전체에 영향을 주지 않기 위함.  
기존에는 가상 머신으로 가상화 했는데 이건 하드웨어까지 가상화 해서 성능이 낮았음.  
컨테이너 가상화는 OS 단계의 가상화이기 때문에 더 가벼워짐.
